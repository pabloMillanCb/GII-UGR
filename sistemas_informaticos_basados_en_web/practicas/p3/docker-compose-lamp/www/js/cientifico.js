...
function sendComment() {
    ...
    var date = new Date();
    const textDate = date.toLocaleDateString()+ " - " + date.getHours() + ":" + date.getMinutes();
    fecha.appendChild(document.createTextNode(textDate))
    ...
    const textval = "\""+document.getElementById("texto-comentario").value+"\""
    const fechaval = textDate

    $.ajax({
        url: '../js_handler.php',
        type: 'post',
        data: { postComentario: "1",
                idCientifico: myVar, 
                nombre: nameval,
                mail: mailval,
                fecha: fechaval,
        ...
        success: function(response) { console.log("respuesta es " + response); }
    });
